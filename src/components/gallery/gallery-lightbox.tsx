      ...
      <div className="columns-1 gap-5 md:columns-2 xl:columns-3">
        {photos.map((photo, index) => (
          <button
            ...
            className="masonry-card group border-border bg-surface/70 mb-5 block w-full overflow-hidden rounded-[1.6rem] border text-left shadow-[0_20px_70px_rgba(16,34,28,0.08)]"
          >
            ...
            <div className="space-y-2 p-4">
              <p className="text-foreground text-sm font-medium">{photo.caption ?? photo.alt}</p>
              {photo.takenAt ? <p className="text-muted text-xs">{photo.takenAt}</p> : null}
            </div>
          </button>
        ))}
      </div>
      ...
      {activePhoto ? (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/78 px-4 py-8 backdrop-blur-xl">
          <button
            ...
            className="absolute top-4 right-4 rounded-full border border-white/10 bg-white/10 p-3 text-white hover:bg-white/15"
          >
            <X className="h-5 w-5" />
          </button>
          <button
            ...
            className="absolute top-1/2 left-4 -translate-y-1/2 rounded-full border border-white/10 bg-white/10 p-3 text-white hover:bg-white/15"
          >
          ...
          <button
            ...
            className="absolute top-1/2 right-4 -translate-y-1/2 rounded-full border border-white/10 bg-white/10 p-3 text-white hover:bg-white/15"
          >
